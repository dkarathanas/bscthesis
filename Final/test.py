...
class Simulation():
    ...
    def init_catchment(self):
        ...
        # print ("Certain Catchment:")
        # print (CC)
        # PC = G.get_probabilistic_catchment()
        # print ("Probabilistic Catchment:")
        # print (PC)

    def analysis(self):
        self.num_of_attacks = 100000
    ...
    def plot_ratio_of_path_lengths(self):
        ...
        data_length = {"no bbx" : self.results["no_bbx_length"],
                "bogon" : self.results["bogon_paths_length"],
                "cones" : self.results["cones_paths_length"]
                # "anycast" : self.results["anycast_paths_length"]
        }
        # print (data)
        data_ratio = {"bogon" : self.results["bogon_paths_ratio"],
                "cones" : self.results["cones_paths_ratio"],
                "anycast" : self.results["anycast_paths_ratio"]
        }

        print ("no_bbx_length = %d", len(self.results["no_bbx_length"]))
        print ("bogon_paths_length = %d", len(self.results["bogon_paths_length"]))
        print ("cones_paths_length = %d", len(self.results["cones_paths_length"]))
        print ("anycast_paths_length = %d", len(self.results["anycast_paths_length"]))


        # df = pd.DataFrame(data_length)
        # boxplot = df.boxplot()
        # plt.title("Paths Length")
        # plt.xlabel("Blackboxes")
        ...
        # df1 = pd.DataFrame(data_ratio)
        # boxplot1 = df1.boxplot()
        # plt.title("Ratio of Paths Length")
        # plt.xlabel("Blackboxes")
        # plt.ylabel("Ratio") 
        # plt.show(boxplot1)
        ...
        plt.boxplot( df.plot.box(grid='True'))
        ...
        value1 = self.results["bogon_paths_ratio"]
        value2 = self.results["anycast_paths_ratio"]
        value3 = self.results["cones_paths_ratio"]
        value4 = self.results["bogon_paths_length"]
        value5 = self.results["anycast_paths_length"]
        value6 = self.results["cones_paths_length"]
        value7 = self.results["no_bbx_length"]
        ...
        plt.title("Ratio of Path Lengths")
        plt.xlabel("Bogon")
        plt.ylabel("Ratio")        
        plt.show()
        ...
        plt.boxplot(box_plot4)
        plt.title("AS Path")
        plt.xlabel("Bogon")
        plt.ylabel("Number of Hops")        
        plt.show()

        plt.boxplot(box_plot5)
        plt.title("AS Path")
        plt.xlabel("Anycast")
        plt.ylabel("Number of Hops")  
        plt.show()

        plt.boxplot(box_plot6)
        plt.title("AS Path")
        plt.xlabel("Cones")
        plt.ylabel("Number of Hops")  
        plt.show()

        plt.boxplot(box_plot7)
        plt.title("AS Path")
        plt.xlabel("no_bbx_length")
        plt.ylabel("Number of Hops")  
        plt.show()
    ...
    def plot_histograms(self):
        ...
        plt.xlabel("Path Length")
        plt.ylabel("Count")  
        plt.hist(y, cumulative=True, label='CDF',
         histtype= 'bar', alpha=0.8, color='k')
        plt.show()
